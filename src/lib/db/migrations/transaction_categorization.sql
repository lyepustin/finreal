...
DECLARE
    ...
    DEFAULT_CATEGORY_ID CONSTANT INTEGER := 1;
    DEFAULT_SUBCATEGORY_ID CONSTANT INTEGER := 1;
BEGIN
    -- Only proceed with auto-categorization if category and subcategory are default values
    IF NEW.category_id = DEFAULT_CATEGORY_ID AND (NEW.subcategory_id IS NULL OR NEW.subcategory_id = DEFAULT_SUBCATEGORY_ID) THEN
